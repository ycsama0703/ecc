...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run unseen-ticker stress tests for after-hours aligned-audio extensions."
    )
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("results/afterhours_audio_unseen_ticker_real"),
    )
...
def main() -> None:
    ...
    for ticker in candidate_tickers:
        ...
        model_specs = {
            ...
            "residual_pre_call_market_plus_a4": ["pre_call_market", "a4"],
            "residual_pre_call_market_plus_a4_plus_qna_lsa": ["pre_call_market", "a4", "qna_lsa"],
            "residual_pre_call_market_plus_a4_plus_qna_lsa_plus_aligned_audio_svd": [
                "pre_call_market",
                "a4",
                "qna_lsa",
                "aligned_audio_svd",
    ...
    if not prediction_rows:
        raise SystemExit("no eligible held-out tickers for unseen-ticker audio stress test")

    ordered_prediction_rows = sorted(prediction_rows, key=lambda item: (item["ticker"], item["year"], item["event_key"]))
    write_csv(output_dir / "afterhours_audio_unseen_ticker_predictions.csv", ordered_prediction_rows)

    model_names = [
        "prior_only",
        "residual_pre_call_market_only",
        "residual_pre_call_market_plus_a4",
    ...
    write_json(output_dir / "afterhours_audio_unseen_ticker_summary.json", summary)
