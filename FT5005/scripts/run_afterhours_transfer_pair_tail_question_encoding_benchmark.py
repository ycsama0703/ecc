...
ENCODINGS = {
    ...
    "question_stop_lsa2_bi": {
        "stop_words": "english",
        "ngram_range": (1, 2),
        "lsa_components": 2,
    },
    "question_stop_lsa4_bi": {
        "stop_words": "english",
        "ngram_range": (1, 2),
        "lsa_components": 4,
    },
    "question_stop_lsa8_bi": {
        "stop_words": "english",
        "ngram_range": (1, 2),
        "lsa_components": 8,
    },
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Benchmark compact encoding variants for the hardest-question local semantic transfer signal."
    )
    parser.add_argument(
        "--temporal-root",
        type=Path,
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("results/afterhours_transfer_pair_tail_question_encoding_benchmark_real"),
    )
...
def build_text_lsa_bundle(
    ...
    *,
    ...
    prefix = f"{text_col}__{('stop' if stop_words else 'nostop')}__{ngram_range[0]}{ngram_range[1]}__lsa"
    feature_names = [f"{prefix}_{idx+1}" for idx in range(n_components)]
...
def load_reference_predictions(path: Path, keys: list[str]) -> dict[str, np.ndarray]:
    ...
    for name, col in [
        ...
        ("pair_tail_factor_route", "pair_tail_factor_route"),
        ("current_tail_question_top1_lsa", "tail_question_top1_lsa"),
    ]:
...
def main() -> None:
    ...
    if not train_rows or not val_rows or not refit_rows or not test_agreement_rows or not test_full_rows:
        raise SystemExit("missing rows for pair-tail question encoding benchmark")
    ...
    for family_name, config in ENCODINGS.items():
        ...
        bundle_meta[family_name] = {
            "stop_words": bool(config["stop_words"]),
            "ngram_range": list(config["ngram_range"]),
            "requested_lsa_components": config["lsa_components"],
    ...
    write_json(output_dir / "afterhours_transfer_pair_tail_question_encoding_benchmark_summary.json", summary)
    write_csv(output_dir / "afterhours_transfer_pair_tail_question_encoding_benchmark_family_overview.csv", family_rows)
    write_csv(output_dir / "afterhours_transfer_pair_tail_question_encoding_benchmark_tuning.csv", tuning_rows)
    write_csv(output_dir / "afterhours_transfer_pair_tail_question_encoding_benchmark_test_predictions.csv", prediction_rows)
    write_csv(output_dir / "afterhours_transfer_pair_tail_question_encoding_component_terms.csv", component_term_rows)
