...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run unseen-ticker stress tests for the strict after-hours pre-call semantic line."
    )
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("results/afterhours_precall_unseen_ticker_real"),
    )
...
def main() -> None:
    ...
    if not prediction_rows:
        raise SystemExit("no eligible held-out tickers for unseen-ticker pre-call semantic stress test")

    ordered_prediction_rows = sorted(prediction_rows, key=lambda item: (item["ticker"], item["year"], item["event_key"]))
    write_csv(output_dir / "afterhours_precall_unseen_ticker_predictions.csv", ordered_prediction_rows)
    ...
    write_json(output_dir / "afterhours_precall_unseen_ticker_summary.json", summary)
