...
FACTOR_FAMILIES = {
    ...
    "answerability_plus_observability": [
        "qa_pair_count",
        "qa_pair_low_overlap_share",
        "qa_pair_answer_forward_rate_mean",
        "qa_multi_part_question_share",
    ...
    "answerability_observability_surprise": [
        "qa_pair_count",
        "qa_pair_low_overlap_share",
        "qa_pair_answer_forward_rate_mean",
        "qa_multi_part_question_share",
...
ALPHA_GRID_DEFAULT = "0.01,0.1,1,10,100,1000"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Benchmark compact answerability / observability / surprise factors as agreement-side "
            "gain gates for the current hard-abstention transfer shell."
        )
    )
    parser.add_argument(
        "--temporal-root",
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("results/afterhours_transfer_answerability_factor_benchmark_lsa4_real"),
    )
    ...
    parser.add_argument("--alphas", default=ALPHA_GRID_DEFAULT)
...
def attach_side_features(
    ...
    for row in rows:
        ...
        if qa_row is not None:
            coverage["with_qa"] += 1
        if panel_row is not None and feature_row is not None and qa_row is not None:
        ...
        row["abs_revenue_surprise_pct"] = abs(float(row["revenue_surprise_pct"]))
        row["abs_eps_gaap_surprise_pct"] = abs(float(row["eps_gaap_surprise_pct"]))
        row["abs_ebitda_surprise_pct"] = abs(float(row["ebitda_surprise_pct"]))
    return coverage
...
def build_factor_scores(
    ...
    corr = 0.0
    if np.std(train_factor) > 0.0 and np.std(train_y) > 0.0:
...
def design_matrix(variant: str, rows: list[dict[str, float | int | str]], factor_scores: np.ndarray) -> np.ndarray:
...
def coeff_rows(model: Pipeline, variant: str) -> list[dict[str, float | str]]:
    ridge = model.named_steps["ridge"]
    feature_names = ["answerability_factor"] if variant == "factor_only" else GEOMETRY_FEATURES + ["answerability_factor"]
    return [
...
def build_full_predictions(
    ...
    predicted_gain_signal: np.ndarray,
) -> tuple[np.ndarray, list[dict[str, float | int | str]]]:
    gain_lookup = {
        str(row["event_key"]): float(signal)
        for row, signal in zip(agreement_rows, predicted_gain_signal.tolist())
    }
    preds = []
    pred_rows = []
    for row in full_rows:
        ...
        if int(row["agreement"]) == 1:
            ...
            pred = agreed if use_agreed == 1 else pre
        else:
            signal = 0.0
            use_agreed = 0
            pred = pre
...
def load_reference_predictions(path: Path, keys: list[str]) -> dict[str, np.ndarray]:
    ...
    for key in [MODEL_PRE_ONLY, MODEL_HARD_ABSTENTION, "geometry_only", "geometry_plus_hybrid"]:
        if not lookup:
            continue
        refs[key] = np.asarray([float(lookup[event_key][key]) for event_key in keys], dtype=float)
    return refs
...
def main() -> None:
    ...
    coverage = attach_side_features(rows, args.panel_csv.resolve(), args.features_csv.resolve(), args.qa_csv.resolve())
    ...
    if not train_rows or not val_rows_agreement or not refit_rows or not test_rows_agreement:
        raise SystemExit("missing train/val/refit/test rows for answerability factor benchmark")
    ...
    for family_name, family_features in FACTOR_FAMILIES.items():
        for method in FACTOR_METHODS:
            train_factor, val_factor, loading_rows, factor_meta = build_factor_scores(
                method, family_features, train_rows, val_rows_agreement
            )
            for variant in VARIANTS:
                ...
                for alpha in alpha_grid:
                    ...
                    row.update(factor_meta)
                    tuning_rows.append(row)
                    if best_r2 is None or val_score["r2"] > best_r2:
                        best_r2 = float(val_score["r2"])
                        best_alpha = float(alpha)
                        best_use_agreed = float(np.mean(val_signal > 0.0))
                        best_model = model
                best_configs.append(
                    {
                        "family": family_name,
                        ...
                        "val_r2": best_r2,
                        "val_agreement_use_agreed_share": best_use_agreed,
    ...
    refit_factor, test_factor, loading_rows, refit_factor_meta = build_factor_scores(
        chosen["method"],
        FACTOR_FAMILIES[chosen["family"]],
        refit_rows,
        test_rows_agreement,
    )
    ...
    final_model = fit_ridge(refit_x, refit_gain, float(chosen["best_alpha"]))
    ...
    significance = {}
    for ref_name, ref_pred in refs.items():
        significance[ref_name] = summarize_significance(
            test_target,
            test_pred,
            ref_pred,
    ...
    pred_rows = []
    ref_cols = [MODEL_PRE_ONLY, MODEL_HARD_ABSTENTION, "geometry_only", "geometry_plus_hybrid"]
    ref_lookup = {key: {name: float(refs[name][idx]) for name in ref_cols if name in refs} for idx, key in enumerate(keys)}
    for idx, row in enumerate(test_pred_rows):
        event_key = str(row["event_key"])
        merged = dict(row)
        for name in ref_cols:
            if name in ref_lookup[event_key]:
                merged[name] = ref_lookup[event_key][name]
    ...
    score_rows = []
    score_rows.append({"model": "answerability_factor_route", **test_score})
    for name in ref_cols:
        if name in refs:
            score_rows.append({"model": name, **metrics(test_target, refs[name])})
    ...
    summary = {
        ...
        "best_factor_meta": refit_factor_meta,
    ...
    write_json(output_dir / "afterhours_transfer_answerability_factor_benchmark_summary.json", summary)
    write_csv(output_dir / "afterhours_transfer_answerability_factor_benchmark_tuning.csv", tuning_rows)
    write_csv(output_dir / "afterhours_transfer_answerability_factor_benchmark_test_predictions.csv", pred_rows)
    write_csv(output_dir / "afterhours_transfer_answerability_factor_benchmark_scores.csv", score_rows)
    write_csv(output_dir / "afterhours_transfer_answerability_factor_benchmark_loadings.csv", loading_rows)
    write_csv(output_dir / "afterhours_transfer_answerability_factor_benchmark_coefficients.csv", coeff_rows(final_model, chosen["variant"]))
